...
public class ResourceCopierImpl implements ResourceCopier {
    ...
    private void logExecutionMetrics(String status) {
    	if(getCommandBuilder() != null) {
    	    if(getCommandBuilder().getContext().containsVariable(JOB_ID)) {
    ...
    private void setFilesListingOnBuilder() throws ExecutionException {
        File fileListing = getFilesListing();
        if(fileListing!=null) {
            getCommandBuilder().setVariable(FILES_LISTING,fileListing );
        }
