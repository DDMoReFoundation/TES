...
public class InvokerHelper {
    ...
    public static String exitStatus(InvokerResult results) {
        return results == null ? "No exit status available due to failed execution" : ("" + results.getExitStatus());
    }
    ...
    public static String output(InvokerResult results) {
        return results == null ? "No output available due to failed execution" : results.getOutputStream();
    }
    ...
    public static String errors(InvokerResult results) {
        return results == null ? "No errors available due to failed execution" : results.getErrorStream();
    }
    ...
    /**
     * Report on the outcome of executing a command.
     * @param commandLine the command
     * @param results the outcome
     */
    private void report(String commandLine, InvokerResult results) {
        final String unavailable = "unavailable (no results)";
        String exitStatus = (results == null) ? unavailable : ("" + results.getExitStatus());
        String stdout =     (results == null) ? unavailable : results.getStdout();
        String stderr =     (results == null) ? unavailable : results.getStderr();
        ...
        if (failed(results)) {
            ...
            logger.debug("Command succeeded: " + commandLine);
    ...
    public boolean copySpecificFiles(String sourceAbsolutePath, String[] filesRelativePath, String targetAbsolutePath) throws ExecutionException {
        ...
        checkIsAbsolutePathSupplied(sourceAbsolutePath);
        checkIsAbsolutePathSupplied(targetAbsolutePath);
     ...
     *
     * @FIXME currently that implementation will only ever return true and if mkdir fails an exception will be thrown. That is dirty API. Or it should return boolean identifying outcome of 'mkdir' or it should return void and throw exception in case of a failure.
     * (of course the former is preferable)
     */
    public boolean mkdir(String absolutePath, int mode) throws ExecutionException {
        checkIsAbsolutePathSupplied(absolutePath);
    ...
    /**
     * Delete the file specified in absolute path forcefully.
     * @param absolutePath
     * @return
     * @throws ExecutionException
     */
    public boolean deleteSpecifiedFile(String absolutePath) throws ExecutionException {
        checkIsAbsolutePathSupplied(absolutePath);
        InvokerResult results = null;
        if(new File(absolutePath).isFile()){
            results = runAndReportFailures("rm -f " + absolutePath);
        }else{
            throw new ExecutionException("The path specified is not a file : "+ absolutePath);
        }
            return success(results);
    }

	private void checkIsAbsolutePathSupplied(String absolutePath) {
		Preconditions.checkArgument(new File(absolutePath).isAbsolute(), "The path of the file to create must be absolute");
	}
}
