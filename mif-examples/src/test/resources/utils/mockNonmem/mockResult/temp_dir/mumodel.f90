...
! LAST UPDATE : DEC/2008 - RECODED IN F90/95 AND RESTRUCTURED 
...
!
      SUBROUTINE MUMODEL(THETAQ,MU)
!      
      USE FSIZES,       ONLY: F_LVR,F_PC,F_PG
! INTEGER        
...
!
      INTEGER(KIND=ISIZE) :: I,IREV,ICALLP,ISMALL,J,NEWIND,NVNT ! GPRD_OLD,GPKD_OLD 
!      
...
!
      IRGG=F_PG+1
      IF(.NOT.ALLOCATED(GU))    ALLOCATE(GU(F_LVR,F_LVR+1))
      IF(.NOT.ALLOCATED(HU))    ALLOCATE(HU(F_LVR,F_LVR))
